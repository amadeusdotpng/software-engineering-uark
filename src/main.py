...
import sys
import itertools
import os # using for audio
import random # also using for audio
...
class MainWindow(QtWidgets.QWidget):
    def __init__(self, db: PlayerDatabase):
        ...
        # audio -- Rowyn
        self.audio_output = QAudioOutput() # creating output
        self.media_player = QMediaPlayer() # loading in and playing audio
        self.media_player.setAudioOutput(self.audio_output) # connecting output and player
        self.audio_output.setVolume(0.7) # 70% vol
    ...
    def start_game(self):
        self.play_music() # to play a random audio
        # TODO: actually start the game, currently just ends the program :P
        self.close()


    # Rowynnnnnnn
    def play_music(self, folder_path = "res/tracks"):
        tracks = [f for f in os.listdir(folder_path) if f.endswith(".mp3")] # getting all tracks
        if not tracks:
            print("No audio tracks found in that folder!")
            return
        chosen = os.path.join(folder_path, random.choice(tracks)) # pick randomly
        self.media_player.setSource(QtCore.QUrl.fromLocalFile(os.path.abspath(chosen))) # converts it into an absolute path, loading into player
        self.media_player.play() # play
